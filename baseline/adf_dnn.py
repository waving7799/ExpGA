...
def dnn_fair_testing(dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter,iter):
    ...
    np.save('../results/'+dataset+'/'+ str(sensitive_param) + '/global_samples_adf_adv{}.npy'.format(iter), np.array(global_disc_inputs_list))
    np.save('../results/'+dataset+'/'+ str(sensitive_param) + '/local_samples_adf_adv{}.npy'.format(iter), np.array(local_disc_inputs_list))
