...
sex_pair_set = conf.sex_pair_set
sex_pair = conf.sex_pair
...
def evaluate_global(train_tests, max_global, sensitive_param, tokenizer, model, dataset):
    ...
    for i in range(max_global):
        ...
        rank = get_shap_values(x_train, inp_t, model, toplabel)
            ...
            try:
                if word not in Skip_Words:
                    ...
                    if len(obj["edges"]) > 0:
                        ...
                        inp_key = set(inp_key)
                        if len(inp_key & attributes_key) > 0 or len(inp_key & sex_pair_set) > 0:
                            seeds.append(inp)
...
def evaluate_local(pop, sens_key, model, tokenizer, target, dataset,x_ori):
    ...
    for i in range(len(pop)):
        ...
        if sens_key in sex_pair_set:
            rep_word = sex_pair[sens_key]
            ret1 = ret1[:index] + [rep_word] + ret1[index + 1:]
        else:
            ret0.insert(index, "male")
            ret1.insert(index, "female")
...
def xai_fair_testing(dataset, sens_param, model_path, max_global, max_local):
    ...
    seed_list, seeds_key = evaluate_global(train_texts, max_global, sens_param, tokenizer, model, dataset)
    ...
    global_path = './new_results/{}-{}-{}-globallist'.format(dataset,sens_param,model_name)
    global_path2 = './new_results/{}-{}-{}-globalkey'.format(dataset,sens_param,model_name)
    ...
    for i in range(len(seed_list)):
        ...
        print('Number of local discovery is:' + str(len(local_disc_inputs_list)))
    ...
    print(
        "Percentage discriminatory inputs - " + str(float(len(local_disc_inputs_list)) / float(len(tot_inputs)) * 100))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('sens_param', "sex", 'sensitive index')
